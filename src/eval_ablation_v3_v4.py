...
# ----------------------------- subset detector -----------------------------
# Focus on dataset-style scaffolding prompts (false-positive hotspot).
...
def eval_one_version(version: str, model_path: str, rows: pd.DataFrame, temperature: float = 0.1) -> Dict[str, Any]:
    """
    Evaluate the pure LLM path for a given system/user template version (e.g., "v3" or "v4").
    Uses NO RAG (ragflag=False) to keep the pure-LLM output comparable for ablation.
    """
    ...
    # Emphasize SAFE=0 performance on this subset (reduce false positives)
...
def build_comparison_df(sub: pd.DataFrame, before: Dict[str, Any], after: Dict[str, Any]) -> pd.DataFrame:
    """
    Per-row comparison table: true label, BEFORE vs AFTER scores/labels, and a small snippet.
    Flags rows where AFTER fixed a BEFORE false positive ("FP→OK") or false negative ("FN→OK").
    """
    rows = []
    for i, row in sub.iterrows():
        y = int(row["label"])
        vBs, vBl = float(before["scores"][i]), int(before["preds"][i])
        vAs, vAl = float(after["scores"][i]),  int(after["preds"][i])

        if y == 0 and vBl == 1 and vAl == 0:
            fix = "FP→OK"
        elif y == 1 and vBl == 0 and vAl == 1:
            fix = "FN→OK"
        else:
            fix = ""
        ...
        rows.append({
            ...
            "true_label": y,
            f"{before['version']}_score": vBs,
            f"{before['version']}_label": vBl,
            f"{after['version']}_score":  vAs,
            f"{after['version']}_label":  vAl,
            "fix_flag": fix
        })
...
# ----------------------------- main script -----------------------------
def main():
    ap = argparse.ArgumentParser("Ablation: System Prompt BEFORE → AFTER")
    ...
    ap.add_argument("--before-version", default="v3", help="Template version for BEFORE (default: v3)")
    ap.add_argument("--after-version",  default="v4", help="Template version for AFTER (default: v4)")
    ...
    ap.add_argument("--out-xlsx", default=None, help="Write results to an Excel file (e.g., reports/ablation_v4.xlsx)")
    ap.add_argument("--out-html", default=None, help="Write results to an HTML file (e.g., reports/ablation_v4.html)")
    args = ap.parse_args()
    ...
    # Evaluate BEFORE (e.g., v3) and AFTER (e.g., v4)
    ...
    if not fixes.empty:
        ...
        for _, row in fixes.iterrows():
            ...
            print(f"Before ({before['version']}) score/label: {row[f'{before['version']}_score']:.3f} / {row[f'{before['version']}_label']}"
                  f"  →  After ({after['version']}) score/label: {row[f'{after['version']}_score']:.3f} / {row[f'{after['version']}_label']}  ({row['fix_flag']})")
    else:
...
# Example: Evaluate scaffolding subset; save both Excel & HTML
# python -m src.eval_ablation_v3 --model_path "models/gguf/llama-2-7b-chat.Q4_K_M.gguf" --csv "data/test.csv" --subset scaffold --before-version v3 --after-version v4 --out-xlsx "reports/ablation_v4.xlsx" --out-html "reports/ablation_v4.html" --limit 20
